...
// Read exactly n bytes with a receive timeout. Returns false on timeout/close.
static bool read_n(cast_conn_t *c, uint8_t *dst, size_t n, int timeout_ms)
{
    int fd = -1;
    ...
    while (got < n) {
        ...
        if (r == ESP_TLS_ERR_SSL_WANT_READ || r == ESP_TLS_ERR_SSL_WANT_WRITE) return false; // timeout
...
{
    uint8_t hdr[4];
    if (!read_n(c, hdr, 4, timeout_ms)) return false;
    ...
    if (!read_n(c, c->recv_buf, len, timeout_ms)) return false;
    return cast_msg_decode(c->recv_buf, len, out);
}

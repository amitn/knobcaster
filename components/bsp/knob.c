...
//
// NB: tried a PCNT watch-point ISR (zero idle CPU) but it regressed — edge-
// triggered, it catches the intra-detent bounce the 4ms poll naturally filters,
// so one direction's +1 got cancelled by a spurious -1 ("volume only goes down").
// The poll is proven on this hardware; keep it.
